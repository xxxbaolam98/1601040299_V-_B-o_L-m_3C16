...
if (isset($_POST['register_button'])) {
    ...
    $location1 = substr($location, 3, 1);
    $location1_pattern = '/^[A-Z]+$/';
    $location2 = substr($location, 0, 3);
    $location2_pattern = '/^[0-9]+$/';
    $location3 = substr($location, 4, 1);
    $location3_pattern = '/^[0-9]+$/';
	...
	if (mysqli_num_rows($result1) >= 1) {
       ...
       if(strlen($name) !== 6) {
       $_SESSION['message'] = " name has to be equal to 6 characters";
                  }
       ...
       $_SESSION['message'] = "invalid name";
           ...
           $_SESSION['message'] = "location has to be 5 characters";
             }

              else if(!preg_match($location1_pattern, $location1) || !preg_match($location2_pattern, $location2) || !preg_match($location3_pattern, $location3)) {
       ...
       $_SESSION['message'] = "invalid teacher's name. teacher  must contain letters, dashes and spaces only and must start with upper case letter";
                   ...
                   else if (strlen($semester) > 11 || !preg_match($semester3_pattern, $semester1[1]) || $semester1[1] < 2018) {
$_SESSION['message'] = "invalid semester";

                  } 

                      else if($startdate <= $end_registration) {
 ...
 <?php 
	...
	<form method="POST" action="admin.php?adminpage=editCourse&IDsubject=<?=$IDsubject;?>&ID=<?=$course_ID;?>" class="beta-form-checkout">
		...
		<div class="form-group">
			<tr>
				<td>start date: </td>
				<td><input type="date" name="start_date" class="form-control" value="<?= $course['start_date']; ?>" required>
				 </td>
			</tr>
		</div>
		...
		<div class="form-group">
		 ...
		 <div class="form-group">
			<tr>
				<td>max students: </td>
				<td><input type="number" max="100" min="2" name="max_student" class="form-control" value="<?= $course['max_student']; ?>" required></td>
			</tr>
		</div>
...
<div class="form-group">
			<tr>
				...
				<td><input type="date" name="end_registration" class="form-control" min="<?=$nowdate;?>" value="<?= $course['end_registration']; ?>"  required>
				 </td>
			</tr>
		</div>
