...
if (isset($_POST['register_button'])) {
    ...
    $name_pattern = '/^[A-Z][a-zA-Z -]+$/';
       ...
       $studentID_pattern1 = substr($student_id, 2, 1);
       $studentID_pattern2 = substr($student_id, 3, 3);
	...
	if (mysqli_num_rows($result1) >= 1) {
		...
		else if(!(is_numeric($student_id)) || $studentID_pattern < 18 || strlen($student_id) !== 10 || $studentID_pattern1 !== "0" || $studentID_pattern2 !== "104") {
   $_SESSION['message'] = "student ID must contain 10 numbers, 2 first numbers must be greater than 17 and four next numbers must be 0104";

      }

      else if(!preg_match($name_pattern, $fullname) || strlen($fullname) > 100) {
       $_SESSION['message'] = "Only alphabets, white space allowed and first letter must be upper case";
                  }
          ...
          $_SESSION['message'] = "password must have at least 1 uppercase, lowercase and special character";			
		}
   ...
   $_SESSION['message'] = "must be a list of numbers and greater than 10 and less than 12";
       ...
       $_SESSION['message'] = "the year must be greater than 2017";            	
			...
			$sql = "INSERT INTO student(student_id, password, full_name, email, address, phone, date_birth, year_begin, gender, student_condition) VALUES('$student_id', '$password','$fullname','$email','$address','$phone','$datebirth','$freshmen_year','$gender', '$condition')";
...
<div class="radio">
  <label><input type="radio" name="condition" value="Reserve" disabled>Reserve</label>
</div>

<div class="radio">
  <label><input type="radio" name="condition" value="Quit" disabled> Quit</label>
</div>
